...
detector is to use a secondary emitter as an active element. We further the
study of microchannel plate photomultipliers (MCPs) as an active element of a
shower-maximum detector. We present test beam results obtained using Photonis
XP85011 and Photek 240 MCPs to detect secondary particles of an electromagnetic
shower. We focus on the use of the multiple pixels on the Photonis MCP in order
to find a transverse two-dimensional shower maxima distribution. A spatial
resolution of 0.8 mm was obtained with an 8 GeV electron beam. A method for
measuring time resolution from the Photonis MCP as a whole is presented, and it
improves the time resolution for individual pixel readouts. The time resolution
is found to be better than 40 ps.
...
Future high-energy physics experiments can benefit substantially from shower-maximum
detectors or calorimeters with improved timing 
capability~\cite{Anderson:2015gha,MCPFastCaloNIMA,Ronzhin:2015pba,MCPShowerMaxPaper}.
%think of more citations for this
...
picoseconds~\cite{MCPFastCaloNIMA,Ronzhin:2015pba,MCPShowerMaxPaper}. Moreover
such devices would be intrinsically radiation hard and would survive the harsh
radiation environments at future hadron colliders. A further advantage of MCP's
is their capability for pixelated readout, allowing for the possibility of a
...
concepts for the ILC~\cite{CALICE} and the Phase-2 upgrade of the CMS 
experiment~\cite{CMSHGCal}, and indicate that such calorimeters have promising 
...
% should draw a diagram of how the channels were distributed among the 4 DRS 
units
...
about $3$ or $4$ pixels, and then becomes flat as we include even more pixels. 
The initial $1/\sqrt{N}$ scaling is encouraging as it indicates that further 
granularity in the transverse may help to improve the time resolution. As the 
majority of the shower is covered by the pixels closest to the center of the 
shower, it is not very surprising that beyond about $4$ pixels, we do not observe any 
improvement from the inclusion of additional pixels because they simply do not contain any 
additional information about the shower.
